...
problem_optim_NACA  = ProblemOptim(n_var = para_interval.shape[0], n_obj = n_obj, 
                                    ...
                                    target_mode_idx=target_mode_idx # the mode on which we'll look for the lowest Uc
)
...
np.savez(f'data/res_target_{target_mode_idx}_'+algorithm_name,resX=res.X, resF = res.F)
